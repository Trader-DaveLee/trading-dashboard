...
const els = {};
window.__desk = { selectTrade: id => selectTrade(id), applySameSetupFilter: () => filterBySelectedSetup(), applySameTickerFilter: () => filterBySelectedTicker(), loadSelectedIntoJournal: () => openSelectedInJournal() };

const ID_LIST = ['nav','metrics','calendar','calendar-title','equity-chart','setup-chart','mistake-list','research-notes','entries','exits','calc-summary','trade-form','trade-id','trade-date','ticker','status','session','side','setup-entry','setup-exit','grade','account-size','risk-pct','leverage','playbook-score','maker-fee','taker-fee','stop-price','stop-type','adjustment','tags','mistakes','emotion','context','thesis','review','artifacts','trade-table','detail','detail-insights','q','f-from','f-to','f-status','f-side','f-session','f-setup','f-tag','f-mistake','sort','ticker-list','setup-entry-list','setup-exit-list','journal-status','draft-saved-at','library-result-count','compact-toggle','review-position','review-breadcrumb','prev-trade','next-trade','filter-same-setup','filter-same-ticker','clear-quick-filter','export-json','import-json-btn','import-json','desk-rules','risk-risk-dollar','risk-qty','risk-margin','risk-notional','risk-stop-distance','risk-fees','risk-slider','toggle-deep-journal','deep-journal-section'];

bootstrap();

function bootstrap() { cacheEls(); bindEvents(); hydrateInitialForm(); render(); }
...
function bindEvents() {
  ...
  els['toggle-deep-journal'].onclick = (e) => {
    ...
    e.target.textContent = els['deep-journal-section'].classList.contains('hidden') ? '📝 딥 저널링 펼치기 (컨텍스트, 실수, 차트 첨부) ▼' : '📝 딥 저널링 접기 ▲';
  };
...
function render() { renderNav(); renderMetaLists(); renderOverview(); renderLibrary(); applyPrefs(); updatePreview(); refreshJournalStatus(); }
function renderNav() { els.nav.innerHTML = views.map(view => `<button class="${state.view === view ? 'active' : ''}" data-view="${view}">${label(view)}</button>`).join(''); els.nav.querySelectorAll('button').forEach(btn => btn.onclick = () => switchView(btn.dataset.view)); }
function switchView(view) { state.view = view; views.forEach(v => document.getElementById(`view-${v}`).classList.toggle('active', v === view)); renderNav(); }
...
function renderCalendar() {
  ...
  for (let d = 1; d <= total; d++) {
    ...
    html += `<div class="day" data-day="${key}"><span class="date">${d}</span><span class="pnl ${pnl > 0 ? 'positive' : pnl < 0 ? 'negative' : 'neutral'}">${pnl ? money(pnl) : ''}</span></div>`;
  }
...
function renderEquity(closed) {
  ...
  const sorted = [...closed].sort((a, b) => new Date(a.date) - new Date(b.date)); let equity = 0; const points = sorted.map((t, i) => ({ x: i, y: (equity += t.metrics.pnl) }));
...
function renderMistakes(closed) {
  ...
  els['mistake-list'].innerHTML = counts.length ? counts.map(item => { const stat = byLabel.get(item.label); return `<div class="note-box"><strong>${escapeHtml(item.label)}</strong><div>빈도 ${item.value}회 · 누적 ${money(stat?.totalPnl || 0)} · 평균 ${Number(stat?.avgR || 0).toFixed(2)}R</div></div>`; }).join('') : emptyState('실수 태그가 없습니다.');
}

function renderResearchNotes(closed) {
  const notes = []; const setups = groupAverageR(closed, t => t.setupEntry).filter(x => x.count >= 2);
  if (setups[0]) notes.push(`가장 유리한 셋업은 <strong>${escapeHtml(setups[0].label)}</strong> (${setups[0].value.toFixed(2)}R) 입니다.`);
  const mistakes = tagStats(closed, t => t.mistakes);
  if (mistakes[0]) notes.push(`가장 비용이 큰 실수는 <strong>${escapeHtml(mistakes[0].label)}</strong> 입니다.`);
...
function renderLibrary() {
  ...
  const selectedExists = rows.some(t => t.id === state.selectedTradeId); if (!selectedExists) state.selectedTradeId = rows[0]?.id || null;
  els['library-result-count'].textContent = `${rows.length}개 결과`; els['compact-toggle'].textContent = state.prefs.compactTable ? '표준 밀도' : '컴팩트 밀도';
  const selectedIndex = rows.findIndex(t => t.id === state.selectedTradeId); els['review-position'].textContent = rows.length ? `${selectedIndex + 1} / ${rows.length}` : '0 / 0';
  els['trade-table'].innerHTML = rows.length ? rows.map(tradeRow).join('') : `<tr><td colspan="12" class="empty-state">결과가 없습니다.</td></tr>`;
  els['trade-table'].querySelectorAll('tr[data-id]').forEach(row => row.onclick = () => selectTrade(row.dataset.id));
  renderDetail();
}

function filteredTrades() {
  const q = els.q.value.trim().toLowerCase(); const from = els['f-from'].value; const to = els['f-to'].value; const status = els['f-status'].value; const side = els['f-side'].value; const session = els['f-session'].value; const setup = els['f-setup'].value.trim().toLowerCase(); const tag = els['f-tag'].value.trim().toLowerCase(); const mistake = els['f-mistake'].value.trim().toLowerCase(); const sort = els.sort.value;
  const arr = state.db.trades.filter(t => {
    ...
    if (q && !hay.includes(q)) return false; if (from && day < from) return false; if (to && day > to) return false; if (status !== 'ALL' && t.status !== status) return false; if (side !== 'ALL' && t.side !== side) return false; if (session !== 'ALL' && t.session !== session) return false; if (setup && !String(t.setupEntry || '').toLowerCase().includes(setup)) return false; if (tag && !(t.tags || []).some(x => x.toLowerCase().includes(tag))) return false; if (mistake && !(t.mistakes || []).some(x => x.toLowerCase().includes(mistake))) return false; return true;
  });
...
function artifactLinks(arr = []) {
  ...
  let html = '';
  if (tvLinks.length) {
    html += `<div class="chart-vault"><h4 style="margin:0 0 10px; color:#fbbf24;">📂 Chart Vault</h4><div class="vault-grid">`;
    tvLinks.forEach((u, i) => {
      const imgId = u.split('/x/')[1].replace('/','');
      html += `<a href="${escapeAttr(u)}" target="_blank" title="Click to view original"><img src="https://s3.tradingview.com/x/${imgId}.png" alt="Chart ${i+1}" loading="lazy"/></a>`;
    });
    html += `</div></div>`;
  }
  if (otherLinks.length) {
    html += `<div style="margin-top:10px;">` + otherLinks.map((u, i) => `<a href="${escapeAttr(u)}" target="_blank" rel="noopener noreferrer">🔗 Evidence Link ${i + 1}</a>`).join('<br>') + `</div>`;
  }
  return html;
}
...
function renderDetail() {
  ...
  els.detail.innerHTML = `
    <div class="detail-actions"><button type="button" onclick="window.__desk.loadSelectedIntoJournal()">Journal에서 폼 불러오기</button></div>
    <div class="detail-box">
      <div class="kv">
        <div>날짜</div><div>${fmtDateTime(trade.date)}</div><div>티커</div><div>${escapeHtml(trade.ticker)}</div>
    ...
    <div class="detail-box"><strong>컨텍스트 & 복기</strong><br><br><span style="color:#94a3b8;">${nl(escapeHtml(trade.context || '-'))}</span><hr style="border:0;border-top:1px dashed #334155;margin:10px 0;"><span style="color:#e2e8f0;">${nl(escapeHtml(trade.review || '-'))}</span></div>
    <div class="detail-box"><strong>실수 태그</strong><div class="chips">${chips(trade.mistakes)}</div></div>`;
  
  const similar = similarTrades(trade);
  els['detail-insights'].innerHTML = `
    <div class="review-grid">
      <div class="review-stat"><span class="eyebrow">Setup Cohort</span><strong>${cohort.count} samples</strong><div>평균 ${cohort.avgR.toFixed(2)}R</div></div>
    ...
    <div class="detail-box" style="margin-top:12px;"><strong>유사 샘플 비교</strong><div class="similar-list">${similar.length ? similar.map(item => `<div class="similar-item" onclick="window.__desk.selectTrade('${item.id}')"><strong>${fmtDateTime(item.date)}</strong><div>${escapeHtml(item.ticker)} · <span class="${item.metrics.r >= 0 ? 'positive' : 'negative'}">${item.metrics.r.toFixed(2)}R</span></div></div>`).join('') : '<span class="neutral">샘플 부족</span>'}</div></div>
  `;
}
...
function deleteTrade() { const id = els['trade-id'].value; if (!id) return; if (!confirm('삭제하시겠습니까?')) return; state.db.trades = state.db.trades.filter(t => t.id !== id); saveDB(state.db); clearDraft(); state.selectedTradeId = null; resetForm(); render(); }
function duplicateTrade() { if (!els['trade-id'].value) return; els['trade-id'].value = ''; els['trade-date'].value = inputDate(new Date().toISOString()); state.dirty = true; switchView('journal'); refreshJournalStatus('복제본 준비됨'); persistDraft(); }
...
function resetForm(options = {}) {
  els['trade-id'].value = ''; els['trade-date'].value = inputDate(new Date().toISOString()); els['stop-price'].value = ''; els.adjustment.value = 0; els.tags.value = ''; els.mistakes.value = ''; els.emotion.value = ''; els.context.value = ''; els.thesis.value = ''; els.review.value = ''; els.artifacts.value = '';
...
function loadTradeIntoForm(trade) {
  ...
  els['setup-entry'].value = trade.setupEntry; els['setup-exit'].value = trade.setupExit; els.grade.value = trade.grade; els['account-size'].value = trade.accountSize; els['risk-pct'].value = trade.riskPct; els.leverage.value = trade.leverage;
...
function renderLegs(kind) {
  ...
  holder.innerHTML = target.map((row, index) => `
    <div class="leg-row">
      ...
      <button type="button" data-remove-kind="${kind}" data-remove-index="${index}" class="btn-del">✕</button>
    </div>`).join('');
...
function updatePreview() {
  ...
  if (metrics.directionError || !metrics.valid) {
    resetRiskPlanner(); els['calc-summary'].innerHTML = '<span style="color:#ef4444;">유효한 진입가, 손절가, 비중 100%가 필요합니다.</span>'; return;
  }
  ...
  els['calc-summary'].innerHTML = `Avg Entry: <strong style="color:#fff;">${num(metrics.avgEntry)}</strong> | Qty: <strong style="color:#fff;">${metrics.qty.toFixed(5)}</strong><br>Net PnL: <strong class="${metrics.pnl >= 0 ? 'positive' : 'negative'}">${money(metrics.pnl)} (${metrics.r.toFixed(2)}R)</strong>`;
}

function resetRiskPlanner() { els['risk-risk-dollar'].textContent = money(0); els['risk-qty'].textContent = '0.00000'; els['risk-margin'].textContent = money(0); els['risk-slider'].textContent = '0.0%'; els['risk-slider'].style.color = '#34d399'; els['risk-notional'].textContent = money(0); els['risk-stop-distance'].textContent = '0.00%'; els['risk-fees'].textContent = money(0); }
...
function clearFilters() { ['q','f-from','f-to','f-setup','f-tag','f-mistake'].forEach(id => els[id].value = ''); ['f-status','f-side','f-session'].forEach(id => els[id].value = 'ALL'); els.sort.value = 'newest'; renderLibrary(); }
function syncMeta(trade) { if (trade.ticker && !state.db.meta.tickers.includes(trade.ticker)) state.db.meta.tickers.push(trade.ticker); if (trade.setupEntry && !state.db.meta.entrySetups.includes(trade.setupEntry)) state.db.meta.entrySetups.push(trade.setupEntry); if (trade.setupExit && !state.db.meta.exitSetups.includes(trade.setupExit)) state.db.meta.exitSetups.push(trade.setupExit); }
function setupCohortStats(trade) { const cohort = state.db.trades.filter(t => t.id !== trade.id && t.status === 'CLOSED' && t.setupEntry === trade.setupEntry && t.side === trade.side); const count = cohort.length; const avgR = count ? sum(cohort.map(t => t.metrics.r)) / count : 0; const winRate = count ? cohort.filter(t => t.metrics.r > 0).length / count * 100 : 0; return { count, avgR, winRate }; }
...
function tradeInsights(trade) { const arr = []; arr.push(`<strong>${trade.setupEntry || 'UNLABELED'}</strong> 셋업 샘플입니다.`); arr.push((trade.mistakes || []).length ? `실수 태그가 <strong>${trade.mistakes.length}개</strong> 있습니다.` : '기록된 실수가 없습니다.'); return arr; }
function legSummary(legs = []) { if (!legs.length) return '<span class="neutral">-</span>'; return legs.map((leg, index) => `#${index + 1} ${num(leg.price)} · ${Number(leg.weight || 0)}%`).join('<br>'); }
...
function applyDraftToForm(draft) {
  els['trade-id'].value = draft.id || ''; els['trade-date'].value = draft.tradeDate || inputDate(new Date().toISOString()); els.ticker.value = draft.ticker || 'BTCUSDT'; els.status.value = draft.status || 'CLOSED'; els.session.value = draft.session || 'NEW YORK'; els.side.value = draft.side || 'LONG'; els['setup-entry'].value = draft.setupEntry || 'BREAKOUT'; els['setup-exit'].value = draft.setupExit || 'TRAIL STOP'; els.grade.value = draft.grade || 'B'; els['account-size'].value = draft.accountSize ?? 10000; els['risk-pct'].value = draft.riskPct ?? 0.5; els.leverage.value = draft.leverage ?? 10; els['playbook-score'].value = draft.playbookScore ?? 5; els['maker-fee'].value = draft.makerFee ?? 0.02; els['taker-fee'].value = draft.takerFee ?? 0.05; els['stop-price'].value = draft.stopPrice ?? ''; els['stop-type'].value = draft.stopType || 'M'; els.adjustment.value = draft.adjustment ?? 0; els.tags.value = draft.tags || ''; els.mistakes.value = draft.mistakes || ''; els.emotion.value = draft.emotion || ''; els.context.value = draft.context || ''; els.thesis.value = draft.thesis || ''; els.review.value = draft.review || ''; els.artifacts.value = draft.artifacts || '';
...
function persistDraft() { const draft = snapshotDraft(); saveDraft(draft); state.prefs.deskRules = els['desk-rules'].value; savePrefs(state.prefs); state.draftMeta = new Date().toISOString(); refreshJournalStatus(); }
...
function refreshJournalStatus(message = '') { const status = message || (state.dirty ? '저장되지 않은 변경사항' : '입력 상태 정상'); els['journal-status'].textContent = status; els['draft-saved-at'].textContent = state.draftMeta ? `임시저장 ${fmtSavedAt(state.draftMeta)}` : '임시저장 없음'; }
function fmtSavedAt(iso) { return new Date(iso).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' }); }
function bindKeyboardShortcuts() { document.addEventListener('keydown', event => { const meta = event.metaKey || event.ctrlKey; if (meta && event.key.toLowerCase() === 's') { event.preventDefault(); if (state.view !== 'journal') switchView('journal'); els['trade-form'].requestSubmit(); return; } if (meta && event.key.toLowerCase() === 'd') { event.preventDefault(); duplicateTrade(); return; } if (meta && event.key.toLowerCase() === 'k') { event.preventDefault(); switchView('library'); els.q.focus(); els.q.select(); return; } if (!meta && event.key.toLowerCase() === 'j' && !isTypingContext(event.target) && state.view === 'library') { event.preventDefault(); stepSelectedTrade(1); return; } if (!meta && event.key.toLowerCase() === 'k' && !isTypingContext(event.target) && state.view === 'library') { event.preventDefault(); stepSelectedTrade(-1); return; } if (!meta && event.key.toLowerCase() === 'n' && !isTypingContext(event.target)) { event.preventDefault(); switchView('journal'); resetForm(); } }); }
function isTypingContext(target) { return ['INPUT','TEXTAREA','SELECT'].includes(target?.tagName); }
function toggleCompactTable() { state.prefs.compactTable = !state.prefs.compactTable; savePrefs(state.prefs); applyPrefs(); renderLibrary(); }
function applyPrefs() { document.body.classList.toggle('compact-table', !!state.prefs.compactTable); }
function lineSvg(points) { const width = 640, height = 260, padX = 28, padY = 20; const ys = points.map(p => p.y); const minY = Math.min(...ys, 0), maxY = Math.max(...ys, 0); const range = maxY - minY || 1; const x = i => padX + (i * (width - padX * 2) / Math.max(1, points.length - 1)); const y = val => height - padY - ((val - minY) / range) * (height - padY * 2); const path = points.map((p, i) => `${i ? 'L' : 'M'} ${x(i)} ${y(p.y)}`).join(' '); const area = `${path} L ${x(points.length - 1)} ${height - padY} L ${x(0)} ${height - padY} Z`; return `<svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none"><line class="grid-line" x1="${padX}" x2="${width-padX}" y1="${y(0)}" y2="${y(0)}"></line><path class="area" d="${area}"></path><path class="line" d="${path}"></path><text class="axis" x="${padX}" y="15">${money(maxY)}</text><text class="axis" x="${padX}" y="${height - 4}">${money(minY)}</text></svg>`; }
function barSvg(data) { const width = 640, height = 240, padX = 28, padY = 20; const max = Math.max(...data.map(d => Math.abs(d.value)), 1); const rowH = (height - padY * 2) / data.length; return `<svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none">${data.map((d, i) => { const w = (Math.abs(d.value) / max) * (width - 180); const y = padY + i * rowH + 6; const x = 150; return `<text class="axis" x="0" y="${y + 12}">${escapeHtml(d.label)}</text><rect class="${d.value >= 0 ? 'bar-pos' : 'bar-neg'}" x="${x}" y="${y}" width="${w}" height="16" rx="6"></rect><text class="axis" x="${x + w + 8}" y="${y + 12}">${d.value.toFixed(2)}R</text>`; }).join('')}</svg>`; }
