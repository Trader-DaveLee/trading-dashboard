...
const ID_LIST = [
  ...
  'view-library','q','f-from','f-to','f-status','f-side','f-session','f-setup','f-tag','f-mistake','sort','clear-filters','library-result-count','review-position','review-breadcrumb','prev-trade','next-trade','filter-same-setup','filter-same-ticker','clear-quick-filter','trade-table','detail','detail-insights'
];
...
function bindEvents() {
  ...
  const inputs = ['trade-date','ticker','status','session','side','setup-entry','setup-exit','account-size','risk-pct','leverage','maker-fee','taker-fee','stop-price','stop-type','adjustment','tags','mistakes','emotion','context','thesis','review','artifacts','desk-rules'];
  inputs.forEach(id => {
    if(els[id]) {
...
function render() { renderNav(); renderOverview(); renderLibrary(); updatePreview(); refreshJournalStatus(); renderQuickTags(); }
...
function renderOverview() {
  ...
  // 중요한 지표는 large, 중간은 medium, 작은 건 small 사이즈로 배치
  const metrics = [
    ...
    { label: 'Total Fees', value: money(s.fees), sub: 'Paid', tone: 'metric-warn', size: 'small' }
  ];
...
function renderResearchNotes(closed) {
  ...
  if (setups[0]) notes.push(`가장 성과가 좋은 셋업은 <strong>${escapeHtml(setups[0].label)}</strong> (${setups[0].value.toFixed(2)}R) 입니다.`);
...
function resetForm(options = {}) {
  ...
  els.context.value = ''; els.thesis.value = ''; els.review.value = ''; els.artifacts.value = '';
...
function resetRiskPlanner() {
  els['risk-risk-dollar'].textContent = money(0); els['risk-qty'].textContent = '0.00000'; els['risk-margin'].textContent = money(0); els['risk-slider'].textContent = '0.0%'; els['risk-slider'].style.color = '#34d399'; els['risk-notional'].textContent = money(0); els['risk-stop-distance'].textContent = '0.00%'; els['risk-fees'].textContent = money(0);
}
...
function applyDraftToForm(draft) {
  ...
  els['account-size'].value = draft.accountSize ?? 10000; els['risk-pct'].value = draft.riskPct ?? 0.5; els.leverage.value = draft.leverage ?? 10;
